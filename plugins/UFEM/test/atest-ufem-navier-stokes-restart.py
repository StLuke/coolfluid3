...
class TaylorGreen:
    ...
    def read_mesh(self, filename):
        reader = self.model.Domain.create_component(
            'CF3MeshReader', 'cf3.mesh.cf3mesh.Reader')
        reader.mesh = self.model.Domain.create_component(
            'Mesh', 'cf3.mesh.Mesh')
    ...
    def set_restart(self, filename):
        if self.builder == 'cf3.UFEM.NavierStokes':
            reader = self.solver.InitialConditions.create_component(
                'Reader', 'cf3.solver.actions.ReadRestartFile')
        else:
            reader = self.solver.InitialConditions.Restarts.create_component(
                'Reader', 'cf3.solver.actions.ReadRestartFile')
    ...
    def setup_model(self, builder):
        ...
        model = cf.Core.root().create_component(
            'NavierStokes'+self.prefix, 'cf3.solver.ModelUnsteady')
        self.model = model
        ...
        self.restart_writer.Writer.file = cf.URI(
            self.prefix+'-{iteration}.cf3restart')
    ...
    def setup(self, Ua, Va, D, theta):
        ...
        series_writer = self.solver.TimeLoop.create_component(
            'TimeWriter', 'cf3.solver.actions.TimeSeriesWriter')
        writer = series_writer.create_component(
            'Writer', 'cf3.mesh.VTKXML.Writer')
        ...
        if self.builder == 'cf3.UFEM.NavierStokes':
            ...
            self.solver.create_fields()
            writer.fields = [mesh.geometry.navier_stokes_p_solution.uri(
                ), mesh.geometry.navier_stokes_u_solution.uri()]
...
tg_impl = TaylorGreen(builder='cf3.UFEM.NavierStokes', dt=dt,
                      element=elem, prefix='restart-implicit-full')
...
tg_impl_restart = TaylorGreen(
    builder='cf3.UFEM.NavierStokes',
    dt=dt, element=elem,
    prefix='restart-implicit-restarted')
...
# this relatively high number is due to the use of the iterative LSS solver
meshdiff.max_ulps = 500000
meshdiff.execute()
if not meshdiff.properties()['mesh_equal']:
    raise Exception('Bad restart for implicit solve')

# Run a simulation, saving restarts every 5 steps
tg_semi_impl = TaylorGreen(builder='cf3.UFEM.NavierStokesSemiImplicit',
                           dt=dt, element=elem, prefix='restart-semi-full')
...
# Restart after the 5 first steps
tg_semi_impl_restart = TaylorGreen(
    builder='cf3.UFEM.NavierStokesSemiImplicit',
    dt=dt,
    element=elem,
    prefix='restart-semi-restarted')
