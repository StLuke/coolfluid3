...
bc.add_constant_bc(
    region_name='symm', variable_name='Velocity').value = u_in
bc.add_constant_bc(
    region_name='wall', variable_name='Velocity').value = [0., 0.]
...
while current_end_time < final_end_time:
    ...
    model.simulate()
    domain.write_mesh(
        cf.URI('navier-stokes-cylinder2d_output-' + str(iteration) + '.pvtu'))
    iteration += 1
    if iteration == 1:
