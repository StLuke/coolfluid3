...
def run_simulation(sf_name):
    ...
    ic_heat = solver.InitialConditions.create_initial_condition(
        builder_name='cf3.UFEM.InitialConditionFunction',
        field_tag='source_terms')
    ...
    mesh_generator = cf.root.create_component(
        "mesh_generator", "cf3.mesh.SimpleMeshGenerator")
    ...
    bc.add_constant_bc(
        region_name='xneg', variable_name='Temperature').value = Tb
    bc.add_constant_bc(
        region_name='xpos', variable_name='Temperature').value = Tb
