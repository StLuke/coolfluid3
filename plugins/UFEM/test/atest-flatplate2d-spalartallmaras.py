...
# Because of multi-region support, solvers do not automatically
# have a region assigned, so we must manually set the solvers to
# work on the whole mesh
...
wall_distance = domain.create_component(
    'WallDistance', 'cf3.mesh.actions.WallDistance')
...
bc.add_constant_bc(
    region_name='inlet',
    variable_name='TurbulentViscosity').options().set('value', NU_in)
bc.add_constant_bc(
    region_name='bottom1',
    variable_name='TurbulentViscosity').options().set('value',  NU_wall)
bc.add_constant_bc(
    region_name='bottom2',
    variable_name='TurbulentViscosity').options().set('value',  NU_wall)
bc.add_constant_bc(
...
while time.end_time < final_end_time:
    ...
    domain.write_mesh(cf.URI(
        'atest-flatplate2d-satm-coupled_limit-' + str(iteration) + '.pvtu'))
    iteration += 1
    if iteration == 1:
