...
# Initial conditions
ic_u = solver.InitialConditions.NavierStokes.create_initial_condition(
...
ic_u.value = ['0', '0']
ic_g = solver.InitialConditions.NavierStokes.create_initial_condition(
    builder_name='cf3.UFEM.InitialConditionFunction', field_tag='body_force')
...
bc_u.add_constant_bc(
    region_name='bottom', variable_name='Velocity').value = [0., 0.]
bc_u.add_constant_bc(
    region_name='top', variable_name='Velocity').value = [0., 0.]
# Pressure BC
ns_solver.PressureLSS.BC.add_constant_bc(
    region_name='center', variable_name='Pressure').value = 0.
...
for ((x, y), (u, v)) in zip(mesh.geometry.coordinates,
                            mesh.geometry.navier_stokes_u_solution):
    u_ref = y*(2-y)
    if abs(u_ref - u) > 1e-2:
        raise Exception(
            'Error in u component: {u} != {u_ref} at y = {y}'.format(
                u=u, u_ref=u_ref, y=y))
    if abs(v) > 1e-8:
        raise Exception(
