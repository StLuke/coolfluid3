...
def make_square(x_segs, y_segs):
    blocks = cf.Core.root(
        ).create_component('blocks', 'cf3.mesh.BlockMesh.BlockArrays')
...
class TestCase:
    def __init__(self, modelname, segments, use_spec):
        if len(sys.argv) == 2:
            ...
            self.nb_procs = 1
        self.model = cf.Core.root(
            ).create_component(modelname, 'cf3.solver.ModelUnsteady')
        self.domain = self.model.create_domain()
        self.physics = self.model.create_physics(
        ...
        self.ns_solver = self.solver.add_unsteady_solver(
    ...
    def grow_overlap(self):
        if self.nb_procs > 1:
            globconn = self.domain.create_component(
                'GlobalConnectivity', 'cf3.mesh.actions.GlobalConnectivity')
            globconn.options().set('mesh', self.mesh)
            globconn.execute()

            grow = self.domain.create_component(
                'GrowOverlap', 'cf3.mesh.actions.GrowOverlap')
    ...
    def square_mesh_triags(self):
        ...
        blocks.partition_blocks(nb_partitions=self.nb_procs, direction=0)
        blocks.create_mesh(self.mesh.uri())
        triangulator = self.domain.create_component(
    ...
    def cube_mesh_hexas(self):
        self.mesh = self.domain.create_component('Mesh', 'cf3.mesh.Mesh')
        blocks = make_square(self.segments[0], self.segments[1])
        blocks.extrude_blocks(
            positions=[1.], nb_segments=[self.segments[2]], gradings=[1.])
        blocks.partition_blocks(nb_partitions=self.nb_procs, direction=0)
        blocks.create_mesh(self.mesh.uri())
        self.setup_lss()
    ...
    def setup_lss(self):
        self.grow_overlap()
        self.ns_solver.options(
            ).set('regions', [self.mesh.access_component('topology').uri()])
    ...
    def run(self):
        ...
        try:
            ...
            print ('<DartMeasurement name=\"' +
                   self.model.name() +
                   ' time\" type=\"numeric/double\">' +
                   str(self.ns_solver.get_child(
                       assembly_name).properties()['timer_mean']) +
                   '</DartMeasurement>')
        except:
