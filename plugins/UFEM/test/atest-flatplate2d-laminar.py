...
# Add initial conditions for the Navier-Stokes solver,
# which uses 'navier_stokes_solution' as a tag for its solution fields
...
bc.add_constant_bc(
    region_name='inlet', variable_name='Velocity').options().value = u_in
bc.add_constant_bc(region_name='bottom1',
                   variable_name='Velocity').options().value = u_wall
bc.add_constant_bc(region_name='bottom2',
                   variable_name='Velocity').options().value = u_wall
bc.add_constant_component_bc(region_name='bottom3', variable_name='Velocity',
                             component=1).options().value = 0.
bc.add_constant_bc(
    region_name='outlet', variable_name='Pressure').options().value = 0.
bc.add_constant_bc(
    region_name='top', variable_name='Velocity').options().value = u_in
...
while time.end_time < final_end_time:
    ...
    model.simulate()
    domain.write_mesh(
        cf.URI('atest-flatplate2d-laminar-' + str(iteration) + '.pvtu'))
    iteration += 1
    if iteration == 1:
