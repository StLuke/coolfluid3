...
class TaylorGreen:
    ...
    def create_mesh(self, segments):
        ...
        blocks = domain.create_component(
            'blocks', 'cf3.mesh.BlockMesh.BlockArrays')
        ...
        bottom_patch = blocks.create_patch_nb_faces(
            name='bottom', nb_faces=1)
        bottom_patch[0] = [0, 1]

        top_patch = blocks.create_patch_nb_faces(name='top', nb_faces=1)
        top_patch[0] = [5, 4]

        right_patch = blocks.create_patch_nb_faces(
            name='right', nb_faces=2)
        right_patch[0] = [1, 3]
        right_patch[1] = [3, 5]

        blocks.partition_blocks(
            nb_partitions=cf.Core.nb_procs(), direction=0)
        ...
        create_point_region = domain.create_component(
            'CreatePointRegion', 'cf3.mesh.actions.AddPointRegion')
        ...
        if self.element == 'triag':
            triangulator = domain.create_component(
                'Triangulator', 'cf3.mesh.MeshTriangulator')
    ...
    def setup_ic(self, u_tag, p_tag):
        ...
        ic_u = ic_comp.create_initial_condition(
            builder_name='cf3.UFEM.InitialConditionFunction', field_tag=u_tag)
        ic_u.variable_name = 'Velocity'
        ic_u.regions = [self.mesh.topology.interior.uri()]
        ic_u.value = [
            '{Ua} - cos(pi/{D}*x)*sin(pi/{D}*y)'.format(
                Ua=self.Ua, D=self.D),
            '{Va} + sin(pi/{D}*x)*cos(pi/{D}*y)'.format(
                Va=self.Va, D=self.D)]

        ic_p = ic_comp.create_initial_condition(
            builder_name='cf3.UFEM.InitialConditionFunction', field_tag=p_tag)
        ic_p.regions = [self.mesh.topology.interior.uri()]
        ic_p.variable_name = 'Pressure'
        ic_p.value = [
            '-0.25*(cos(2*pi/{D}*x) + cos(2*pi/{D}*y))'.format(D=self.D)]
    ...
    def setup_model(self):
        ...
        model = cf.Core.root(
            ).create_component('NavierStokes', 'cf3.solver.ModelUnsteady')
        self.model = model
    ...
    def add_pressure_bc(self, bc, var_name='Pressure'):
        ...
        bc.add_function_bc(
            region_name='center',
            variable_name=var_name).value = ['-0.25 * (cos(2*pi/{D}*(x - {Ua}*(t+{dt}))) + cos(2*pi/{D}*(y - {Va}*(t+{dt})))) * exp(-4*{nu}*pi^2/{D}^2*(t+{dt})) '.format(D=self.D, nu=nu, Ua=self.Ua, Va=self.Va, dt=self.dt)]
    ...
    def setup_implicit(self, segments, Ua, Va, D, theta):
        ...
        #   Amesos.solver_type = 'Mumps'
        #lss.SolutionStrategy.Parameters.LinearSolverTypes.\
        #   Amesos.create_parameter_list('Amesos Settings').\
        #   add_parameter(name = 'MaxProcs', value=1)
        #lss.SolutionStrategy.Parameters.LinearSolverTypes.\
        #   Amesos.AmesosSettings.add_parameter(name = 'Redistribute', \
        #   value=True)
        #lss.SolutionStrategy.Parameters.LinearSolverTypes.\
        #   Amesos.AmesosSettings.create_parameter_list('Mumps').\
        #   add_parameter(name='Equilibrate', value = False)

        solver.create_fields()
        self.setup_ic('navier_stokes_solution', 'navier_stokes_solution')

        # kinetic_energy = solver.add_unsteady_solver\
        #   ('cf3.UFEM.KineticEnergyIntegral')
        # kinetic_energy.regions = [mesh.topology.interior.uri()]
        # kinetic_energy.history = solver.create_component('KEHistory', \
        #      'cf3.solver.History')
    ...
    def setup_semi_implicit(self, segments, Ua, Va, D, theta):
        ...
        solver = self.setup_model()
        ns_solver = solver.add_unsteady_solver(
        ...
        #ns_solver.PressureLSS.solution_strategy = \
    ...
    def iterate(self, numsteps, save_interval=1, process_interval=1):
        ...
        while time.current_time < final_end_time:
            time.end_time += tstep
            self.model.simulate()
            #self.model.Solver.TimeLoop.NavierStokesExplicit.InnerLoop.\
            #   PressureSystem.LSS.print_system('pressure_system.plt')
            ...
            if self.iteration % save_interval == 0:
                self.model.Domain.write_mesh(cf.URI(
                    self.basename+'/taylor-green-' +
                    str(self.iteration) +
                    '.pvtu'))
            self.iteration += 1
            if self.iteration == 1:
                self.model.Solver.options.disabled_actions = [
                    'InitialConditions']
        self.outfile.close()
    ...
    def check_result(self, dumpfile=False):
        ...
        for i in self.sample_coords:
            ...
            (u_num[i], v_num[i], p_num[i]) = (u_sol[i][u_idx],
                                              u_sol[i][v_idx], p_sol[i][p_idx])
            u_th[i] = Ua - np.cos(np.pi/D*(x-Ua*t))*np.sin(
                np.pi/D*(y-Va*t))*np.exp(-2.*nu*np.pi**2/D**2*t)
            v_th[i] = Va + np.sin(np.pi/D*(x-Ua*t))*np.cos(
                np.pi/D*(y-Va*t))*np.exp(-2.*nu*np.pi**2/D**2*t)
            p_th[i] = -0.25 * (np.cos(2*np.pi/D*(x - Ua*t)) + np.cos(
                2*np.pi/D*(y - Va*t)))*np.exp(-4.*nu*np.pi**2/D**2*t)
        ...
        self.outfile.write('{t},{u},{v},{p}'.format(
            t=t,
            u=self.max_error[0, self.iteration],
            v=self.max_error[
                1,
                self.iteration], p=self.max_error[2, self.iteration]))
        for i in self.probe_points:
            self.outfile.write(',{u},{v},{p}'.format(u=u_th[i] - u_num[i],
                               v=v_th[i] - v_num[i], p=p_th[i] - p_num[i]))
        self.outfile.write('\n')
        if self.iteration > 3:
...
taylor_green_impl.setup_implicit(
    options.segs, 0.3, 0.2, D=0.5, theta=options.theta)
taylor_green_impl.iterate(options.tsteps, 1, 1)

taylor_green_semi = TaylorGreen(dt=options.dt, element=options.elem)
taylor_green_semi.setup_semi_implicit(
    options.segs, 0.3, 0.2, D=0.5, theta=options.theta)
