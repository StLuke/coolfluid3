...
# This test runs two separate heat conduction problems
# on two different regions of the same mesh
...
hc_bottom.options(
    ).set('regions', [mesh.access_component('topology/solid_bottom').uri()])
hc_top.options(
    ).set('regions', [mesh.access_component('topology/solid_top').uri()])

# Boundary conditions for the bottom part
bc_bot = hc_bottom.get_child('BoundaryConditions')
bc_bot.options().set('regions', [mesh.access_component(
    'topology').uri()])  # needed to make the lookup work
bc_bot.add_constant_bc(region_name='bottom',
                       variable_name='Temperature').options().set('value', 10)
# the boundary between the regions had an automatically generated region,
# named 'region_bnd_<name of first region>_<name of second region>
bc_bot.add_constant_bc(region_name='region_bnd_solid_bottom_solid_top',
                       variable_name='Temperature').options().set('value', 50)

# Boundary conditions for the top part
bc_top = hc_top.get_child('BoundaryConditions')
bc_top.options().set('regions', [mesh.access_component(
    'topology').uri()])  # needed to make the lookup work
bc_top.add_constant_bc(
    region_name='top', variable_name='Temperature').options().set('value', 10)
bc_top.add_constant_bc(region_name='region_bnd_solid_top_solid_bottom',
                       variable_name='Temperature').options().set('value', 50)
