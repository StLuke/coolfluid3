...
if abs(pressure_integral.result[0] - 1.) > 1e-10:
    raise Exception('Wrong integration result: ' + str(
        pressure_integral.result[0]) + ', expected 1')

velocity_integral = solver.add_unsteady_solver('cf3.UFEM.SurfaceIntegral')
velocity_integral.set_field(
    variable_name='Velocity', field_tag='navier_stokes_solution')
velocity_integral.regions = [mesh.topology.access_component(
    'left').uri(), mesh.topology.access_component('bottom').uri()]
