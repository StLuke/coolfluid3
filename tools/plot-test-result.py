...
# expects a list of lines that are ordered:
#       title, X-values, Y-values, legend label
varnames = []
for line in p.stdout:
    if line.startswith(var_prefix):
        ...
        if len(varnames) == 3:
            pylab.plot(locals()[varnames[0]], locals()[
                       varnames[1]], label=locals()[varnames[2]])
...
print '<DartMeasurementFile name="Graph" type="image/png">' + os.path.join(
    os.path.dirname(test_command), fig_filename) + '</DartMeasurementFile>'
pylab.clf()
