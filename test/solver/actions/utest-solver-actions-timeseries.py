...
series_writer = domain.create_component(
    'SeriesWriter', 'cf3.solver.actions.TimeSeriesWriter')
...
advance_time = domain.create_component(
    'AdvanceTime', 'cf3.solver.actions.AdvanceTime')
...
#and if they do read and compare to reference mesh
...
for i in range(10):
    if i != 0:
        filename = file_template.format(
            time=float(i)*time.time_step, iteration=i)
    else:
